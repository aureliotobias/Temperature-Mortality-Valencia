...
par(mex=0.8,mfrow=c(2,1))
  ...
  plot(data$date, data$tmax, 
       type="l", col= "red",
       ylim=c(5,42),
       ylab="Temperature (ÂºC)", xlab="Date")
...
###########################################################################################################
### Figure 2.                                                                                           ###
### Relative risk (RR) of mortality along daily temperature and lag dimension with reference at 21ÂºC;   ###
### plot of RR for temperature of 28ÂºC at specific lags (bottom-left panel);                            ###
...
l <- layout(matrix(c(1, 1, 2, 3, 4, 5), 
  ...
  temp3d <- plot(predcen, shade=0.02, col=grey(1),
                 ...
                 ylab="Lag", xlab="Temperature (ÂºC)", zlab="RR")
